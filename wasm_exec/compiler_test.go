...
func compileAndRunJsWasm(ctx context.Context, goSrc string, args, environ []string) (stdout, stderr string, exitCode uint32, err error) {
	...
	we, newErr := wasm_exec.NewWasmExec(ctx, r, compiled, wazero.NewModuleConfig().
		WithStdout(stdoutBuf).WithStderr(stderrBuf))
	if newErr != nil {
		err = newErr
		return
	}
	exitCode, err = we.Main(ctx, args, environ)
