...
func writeArgsAndEnviron(ctx context.Context, mem api.Memory, args, environ []string) (argc, argv uint32, err error) {
	...
	strPtr := func(val, field string, i int) (ptr uint32) {
		...
		ptr = offset
		requireWrite(ctx, mem, field, offset, append([]byte(val), 0))
	...
	for _, ptr := range argvPtrs {
		...
		requireWriteUint64Le(ctx, mem, "argvPtrs[i]", offset, uint64(ptr))
		offset += 8
	}
